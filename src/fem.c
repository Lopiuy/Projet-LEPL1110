...
double* femFullSystemEliminate(femFullSystem *mySystem)
{
    ...
    for (k = 0; k < size/2; k++) {
        if ((A[2*k][2*k] == 0) && (A[2*k+1][2*k+1] == 0)) {
            printf("Wraning : disconnected node %d\n", k);
            A[2*k][2*k] = 1;
            A[2*k+1][2*k+1] = 1;
        }
    }
...
int compareNodePos (const void * firstNode, const void * secondNode){
    int * first = (int*) firstNode;
    int * second = (int*) secondNode;
    double diff = GlobalArray[*first] - GlobalArray[*second];
    return (diff < 0) - (diff > 0); // Retourne -1, 0 ou 1
}

void femMeshRenumber (femNodes * theMesh, femRenumType renumType){
    int * inverse = (int*) malloc(sizeof(int) * theMesh->nNodes);
    ...
    switch (renumType){
        case FEM_NO :
            break;
        case FEM_XNUM :
            GlobalArray = theMesh->X;
            qsort(inverse, theMesh->nNodes, sizeof(int), compareNodePos);
            break;
        case FEM_YNUM :
            GlobalArray = theMesh->Y;
            qsort (inverse, theMesh->nNodes, sizeof(int), compareNodePos);
            break;
...
int femMeshComputeBand (femMesh * theMesh){
    ...
    for (int elem_pos = 0; elem_pos < theMesh->nElem; elem_pos++) {
        for (int j = 0; j < nLocal; ++j){
            map[j] = theMesh->nodes->number[theMesh->elem[elem_pos*nLocal + j]];
        }
        ...
        if (band_width < (max - min)){
            band_width = max - min;
        }
    }
    return 2*(band_width + 1);
}
...
void BandMatrix_to_csv(femBandSystem* mySystem){
    FILE *f = fopen("BandMatrix.csv", "w+");
    for (int i = 0; i < mySystem->size; i++){
        for (int j = 0; j < mySystem->size; j++){
            if (j < i){
                fprintf(f, "%lf", 0.0);
            }
            else if (abs(i - j) > mySystem->band){         // If the element is not in the band, we set it to 0.0
                fprintf(f, "%lf", 0.0);
            }
            else{
                fprintf(f, "%lf", mySystem->A[i][j]);
            }
            if (j != mySystem->size-1){
                fprintf(f, ",");
            }
        }
        fprintf(f, "\n");
...
void FullMatrix_to_csv(femFullSystem* mySystem, char* name){
    FILE *f = fopen(name, "w+");
    for (int i = 0; i < mySystem->size; i++){
        for (int j = 0; j < mySystem->size; j++){
...
void femDenumber(femNodes* theNodes, int size, double* solution){
    int i;
    double inverseB[size];
    for (i = 0; i < size; i++) {
        inverseB[i] = solution[2 * theNodes->number[i / 2] + i % 2];
    }
    memcpy(solution, inverseB, size * sizeof(double));
}
...
void  femBandSystemConstrain(femBandSystem *myBandSystem, int myNode, double myValue)
{
    ...
    int lBound = (myNode >= band) ? myNode - band + 1 : 0;
    for (i = lBound ; i < myNode; i++) {
        B[i] -= myValue * A[i][myNode];
        A[i][myNode] = 0;
    }

    int uBound = (myNode + band >= size) ? size : myNode + band;
    for (i=myNode+1; i < uBound ; i++) {
...
{
    for (int i = 0; i < theEdges->nElem; i++) {
        if ((theEdges->elem[2*i] == node || theEdges->elem[2*i+1] == node) && i != elem) {
            return i;
        }
    }
}
...
{    
    ...
    switch (theProblem->solverType) {
        case FEM_FULL : printf("     Full matrix solver \n"); break;
        case FEM_BAND : printf("     Band matrix solver \n"); break;
    }
    printf(" ======================================================================================= \n\n");
}
...
femProblem* femElasticityRead(femGeo* theGeometry, const char *filename)
{
    ...
    int size = 2*theGeometry->theNodes->nNodes;
    theProblem->constrainedNodes = malloc(size*sizeof(int));
    theProblem->ntConditions = calloc(size, sizeof(double));
    for (int i=0; i < size; i++)
        theProblem->constrainedNodes[i] = -1;
    ...
    while (!feof(file)) {
        ...
        if (strncasecmp(theLine,"Solver Type         ",19) == 0) {
            ErrorScan(fscanf(file,":  %[^\n]s \n",(char *)&theArgument));
            if (strncasecmp(theArgument,"Full matrix solver",13) == 0)
                theProblem->solverType = FEM_FULL;
            if (strncasecmp(theArgument,"Band matrix solver",13) == 0)
                theProblem->solverType = FEM_BAND; }
        if (strncasecmp(theLine,"Renumbering         ",19) == 0) {
            ErrorScan(fscanf(file,":  %[^\n]s \n",(char *)&theArgument));
            if (strncasecmp(theArgument,"No renumbering",13) == 0)
                theProblem->renumType = FEM_NO;
            if (strncasecmp(theArgument,"x-num renumbering",13) == 0)
                theProblem->renumType = FEM_XNUM;
            if (strncasecmp(theArgument,"y-num renumbering",13) == 0)
                theProblem->renumType = FEM_YNUM;}
        ErrorScan(fscanf(file,"\n")); }
 
    int iCase = theProblem->planarStrainStress;
    int iSolver = theProblem->solverType;
    int iRenumbering = theProblem->renumType;
    ...
    switch (iRenumbering) {
        case FEM_NO: break;
        case FEM_XNUM: femMeshRenumber(theGeometry->theNodes,FEM_XNUM); break;
        case FEM_YNUM: femMeshRenumber(theGeometry->theNodes,FEM_YNUM); break;
        default: Error("Unexpected renumbering type"); break;
    }

    switch (iSolver) {
        case FEM_FULL :
            theProblem->system   = femFullSystemCreate(size); break;
        case FEM_BAND :
            band = femMeshComputeBand(theGeometry->theElements);
            theProblem->system   = femBandSystemCreate(size,band); break;
        default: Error("Unexpected solver type"); break;
    }
