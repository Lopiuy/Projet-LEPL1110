                  ...
                  double E, double nu, double rho, double g, femElasticCase iCase, femSolverType iSolver, femRenumType iRenum)
{
    ...
    theProblem->g   = g;
    theProblem->rho = rho;
    theProblem->solverType = iSolver;
    theProblem->renumType  = iRenum;
...
void femElasticityWrite(femProblem *theProblem, const char *filename) 
{
   ...
   for(int i=0; i < theProblem->nBoundaryConditions; i++) {
    ...
    switch (theProblem->solverType) {
       case FEM_FULL : fprintf(file,"Solver type        :  Full matrix solver \n"); break;
       case FEM_BAND : fprintf(file,"Solver type        :  Band matrix solver \n"); break;
    }

    switch (theProblem->renumType) {
        case FEM_NO : fprintf(file,"Renumbering        :  No renumbering \n"); break;
        case FEM_XNUM : fprintf(file,"Renumbering        :  x-num renumbering \n"); break;
        case FEM_YNUM : fprintf(file,"Renumbering        :  y-num renumbering \n"); break;
    }
   fclose(file);
}
